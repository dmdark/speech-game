...
$app->get('/words/{category}/{difficulty}/{count}', function($category, $difficulty, $count) use ($app) {
  $words = file_get_contents('words.json');
  $words = json_decode($words, true);
  $_words = array_filter($words, function ($curr) use ($category, $difficulty) {
...
$app->get('/categories', function () use ($app, $conn) {
  ...
  $categories = array_map(function($curr) {
    return [
      'id' => $curr['id'],
      'title' => $curr['title_en'],
...
$app->post('/score', function (Request $request) use ($app, $conn) {
  $userId = $request->get('userId');
  $name = $request->get('userName');
  $score = $request->get('score');
  $categoryId = $request->get('categoryId');

  $result = $conn->query("INSERT INTO score (interest_id, user_id, `name`, score) VALUES ({$categoryId}, {$userId}, '{$name}', {$score})");
...
$app->get('/top', function () use ($app, $conn) {
  $result = $conn->query('SELECT * FROM interest;');
  ...
  $scores = array_map(function($curr) use ($categories) {
    return [
      'name' => $curr['name'],
